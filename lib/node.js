...
function start (processUniqueId) {
  ...
  const randPort = 10000 + Math.round(Math.random() * 5000)
  log({process: process.pid, ipfsId, randPort})
  ...
  const ipfsOptions = {
    ...
    config: {
      Addresses: {
        Swarm: [
          `/ip4/0.0.0.0/tcp/${randPort}`,
          `/ip4/127.0.0.1/tcp/${randPort + 1}/ws`,
  ...
  ipfs.once('ready', async () => {
    ...
    ipfsId = id.id
    log('ipfs::ready, will start room')
    room.start(ipfs, `${topic}`)
  })
  ...
  // setTimeout(() => { room.start(ipfs, `${topic}`) }, 10000)
